...
object ChatClient {
    ...
    final case class SendMessage(content: String) extends Command
    ...
    unreachables.onChange{(ns, _) =>
        Platform.runLater {
            ChatClientApp.gchatControl.map (x => x.updateList(members.toList.filter(y => ! unreachables.exists (x => x == y.ref.path.address))) )
        }
    }

    members.onChange{(ns, _) =>
        Platform.runLater {
            ChatClientApp.gchatControl.map (x => x.updateList(ns.toList.filter(y => ! unreachables.exists (x => x == y.ref.path.address))) )
        }
    }
    ...
    final case class Message(msg: String, from: User) extends Command
    ...
    //groupMessageStarted is called after the server has added the user into the member list in ChatServer
    def groupMessageStarted(): Behavior[ChatClient.Command] = Behaviors.receive[ChatClient.Command] { (context, message) =>
        message match{

            case SendMessage(content) =>
                members.map (x => x.ref ! Message(content, new User(nameOpt.get, context.self)))
                Behaviors.same

            case Message(msg, from) =>
                Platform.runLater {

                    val DATE_PATTERN = "h:mm a"
                    val DATE_FORMATTER = DateTimeFormatter.ofPattern(DATE_PATTERN)
                    var timestamp = DATE_FORMATTER.format(LocalDateTime.now)

                    ChatClientApp.gchatControl.map (x => x.addText(from.name + ": " + msg + " (" + timestamp + ")"))
                }
                Behaviors.same
            ...
            // multicast to all the existing chatclient
            ...
            case ReachabilityChange(reachabilityEvent) =>
                reachabilityEvent match {
                    ...
                    case UnreachableMember(gmember) =>
                        unreachables += gmember.address
                        Behaviors.same

                    case ReachableMember(gmember) =>
                        unreachables -= gmember.address
                        Behaviors.same
