...
object MyConfiguration {
    ...
    def askMyConfig(): Config = {

        runLocalOnly = Some(true)
        localAddress = Some(InetAddress.getLocalHost())

        MyConfiguration(localAddress.get.getHostAddress(), "", scala.io.StdIn.readLine("Enter Port="))
    }

    //Can be used in the future depending on the development
    // def askDevConfig(): Config = {
    //     val ipRaw = Array(192, 168, 0, 175)
